...
The assembly has 83 named parts: a split housing with a keypad and heading display
on its cover, a 1605 ball screw and nut on a guided carriage, a brushless planetary
gearmotor with a 1:1 belt, an ESP32 and ODrive S1 controller with a 12 to 24 V
converter, and the tiller pin and seat socket it mounts on. It is shown on its
own, as a product shot; the seat and tiller are not drawn.
...
screw axis, and the pin's shoulder puts the tiller top 104.5 mm above the seat, against the
...
overlaps between fixed parts are deliberate seatings: the bezel accent and the
ball track sunk into the screw. In the exploded
...
and axis, and a `presentation` block for what only the website draws: the text on the
nameplate and the heading display (each exploding with the part named in
`explodeAs`), the power cable path and the camera target. GLB positions are in meters, under
